...
def load_baseline(path: Path) -> Dict[Tuple[str, str], dict]:
    ...
    with path.open("r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            # Store aggregated baseline stats keyed by (task, metric).
...
def main() -> None:
    ...
    with output_path.open("w", newline="", encoding="utf-8") as f:
        ...
        # Stable ordering keeps diffs deterministic across reruns/environments.
        for rec in sorted(post_records, key=lambda r: (r["adapter"], r["task"], r["metric"])):
            ...
            # Delta convention: delta = post - pre (interpretation depends on metric).
